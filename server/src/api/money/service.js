...
const getMoney = async (userId) => {
    const finalTransaction = await Transactions.findOne({ userId }).sort({ _id: -1 });
...
const addTransaction = async (title, type, sumOperation, userId) => {
    ...
    const finalMoney = !finalTransaction ? 0 : parseInt(finalTransaction.currentMoney);
    const currentMoney = finalMoney + sumOperation;
    const transaction = await Transactions.create({ title, currentMoney, type, sumOperation, userId, });
